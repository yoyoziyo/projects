...
function setupUnits(config){
  const bar=qs('[data-unit-bar]');const select=qs('[data-unit-select]');const summary=qs('[data-unit-summary]');const units=config.units||[];
  if(!bar||!select||!units.length){if(bar)bar.hidden=true;return}
  const storageKey=`yoites:selected-unit:${location.pathname}`;
  select.innerHTML=units.map(unit=>`<option value="${unit.id}">${unit.name} · ${unit.district}</option>`).join('');
  const saved=localStorage.getItem(storageKey);select.value=units.some(unit=>unit.id===saved)?saved:units[0].id;
  const update=()=>{const unit=units.find(item=>item.id===select.value)||units[0];localStorage.setItem(storageKey,unit.id);summary.textContent=`${unit.name} · ${unit.district}`;document.dispatchEvent(new CustomEvent('yoites:unit-change',{detail:unit}))};
  select.addEventListener('change',update);bar.hidden=false;update();
}
...
async function bootstrap(){
  ...
  try{
    ...
    config=configData;applyConfig(config);setupUnits(config);renderCatalog({categories,products},config,(product,variant)=>{addItem(product,variant);showToast(`${product.name} · ${variant.label} adicionado`);const badge=qs('[data-cart-count]');badge.classList.remove('bump');requestAnimationFrame(()=>badge.classList.add('bump'))});

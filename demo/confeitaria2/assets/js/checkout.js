import {getCart,cartTotal,clearCart} from './store.js';
...
const selectedUnit=config=>{const units=config.units||[];const saved=localStorage.getItem(`yoites:selected-unit:${location.pathname}`);return units.find(unit=>unit.id===saved)||units[0]};
...
function buildMessage(config,data,method,unit){
  ...
  return [
    ...
    '🏪 *Unidade*','',
    unit?`${unit.name} · ${unit.district}`:'Unidade não informada',
    unit?.address||'','',
    '👤 *Contato*','',
    `Nome: ${data.get('name')}`,
...
export function setupCheckout(config){
  ...
  form.addEventListener('change',event=>{if(event.target.name==='delivery')toggleAddress();if(event.target===scheduleToggle)toggleSchedule();if(event.target===scheduleDate)updateCheckoutSummary(config)});document.addEventListener('yoites:unit-change',()=>updateCheckoutSummary(config));toggleAddress();toggleSchedule();
  form.addEventListener('submit',event=>{
    event.preventDefault();if(!form.reportValidity()||!getCart().length)return;
    const data=new FormData(form);const method=methodById(config,data.get('delivery'));const unit=selectedUnit(config);
    const destination=(unit?.whatsapp||(config.demoMode?config.demoWhatsapp:config.contact.whatsapp)).replace(/\D/g,'');
    const whatsappUrl=`https://wa.me/${destination}?text=${encodeURIComponent(buildMessage(config,data,method,unit))}`;
...
export function updateCheckoutSummary(config,methodId){
  const form=qs('[data-checkout-form]');const method=methodById(config,methodId||form?.elements.delivery?.value);const unit=selectedUnit(config);const subtotal=cartTotal();const fee=deliveryFee(method);const total=subtotal+fee;

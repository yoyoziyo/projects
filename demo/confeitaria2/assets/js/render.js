...
export function applyConfig(config){
  ...
  document.querySelectorAll('[data-brand-logo]').forEach(logo=>{if(business.logo){logo.src=business.logo;logo.alt=`Logo ${business.name}`}else logo.remove()});
  ...
  const hero=qs('[data-hero]');const heroBanner=qs('[data-hero-banner]');if(content.hero.image){hero.style.setProperty('--hero-banner',`url("${content.hero.image}")`);hero.classList.add('has-banner');heroBanner?.classList.add('has-banner')}else{hero.classList.remove('has-banner');heroBanner?.classList.remove('has-banner')}
...
function card(product,category,config,onAdd){
  ...
  optionsForm.addEventListener('submit',event=>{validateChoiceGroups();if(!optionsForm.reportValidity()){event.preventDefault();return}optionsConfirmed=true;const chosen=selectedVariant().label;optionsTrigger.textContent=chosen;optionsTrigger.title=chosen;optionsTrigger.setAttribute('aria-label',`Alterar opções de ${product.name}: ${chosen}`);updatePrice()});
  updatePrice();

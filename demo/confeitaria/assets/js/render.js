...
export function applyConfig(config){
  ...
  const hero=qs('[data-hero]');hero.style.backgroundImage=`url('${content.hero.image}')`;
...
function productImageUrl(url,width){if(url.startsWith('.'))return url;const parsed=new URL(url);parsed.searchParams.set('w',width);return parsed.toString()}
...
function card(product,category,config,onAdd){
  const node=qs('#product-card-template').content.cloneNode(true);const img=qs('.product-image',node);const variants=getVariants(product);const select=qs('.variant-select',node);const price=qs('.product-price',node);
  img.src=productImageUrl(product.image,700);img.srcset=`${productImageUrl(product.image,420)} 420w, ${productImageUrl(product.image,700)} 700w`;img.sizes='(min-width:1100px) 16vw, (min-width:700px) 23vw, 78vw';img.width=700;img.height=525;img.alt=product.imageAlt||product.name;

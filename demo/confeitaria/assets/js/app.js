...
applyConfig(config);renderCatalog(catalog,config,product=>{addItem(product);showToast(`${product.name} adicionado à sacola`);const badge=qs('[data-cart-count]');badge.classList.remove('bump');requestAnimationFrame(()=>badge.classList.add('bump'))});setupCheckout(config);
...
subscribe(items=>{qs('[data-cart-count]').textContent=items.reduce((sum,i)=>sum+i.quantity,0);qs('[data-cart-total]').textContent=money(cartTotal(),config.currency,config.locale);const box=qs('[data-cart-items]');if(!items.length){box.innerHTML='<div class="empty-state"><strong>Sua sacola está vazia</strong><span>Escolha seus favoritos para começar.</span></div>';return}box.innerHTML=items.map(item=>`<article class="cart-item" data-id="${item.id}"><img src="${item.image}" alt=""><div><h3>${item.name}</h3><p>${money(item.price,config.currency,config.locale)}</p><div class="quantity"><button data-delta="-1" aria-label="Diminuir">−</button><strong>${item.quantity}</strong><button data-delta="1" aria-label="Aumentar">+</button></div></div><button class="remove-item" data-remove aria-label="Remover">×</button></article>`).join('')});

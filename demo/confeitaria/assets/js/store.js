...
export function addItem(product,variant){const id=`${product.id}::${variant.id}`;const current=items.find(item=>item.id===id);if(current)current.quantity+=1;else items.push({id,productId:product.id,variantId:variant.id,variantLabel:variant.label,name:product.name,price:variant.price,image:product.image,quantity:1});save()}

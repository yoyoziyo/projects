const STORAGE_KEY = 'yoites-catalog-cart-v1';
...
export function addItem(product){const current=items.find(item=>item.id===product.id);if(current)current.quantity+=1;else items.push({id:product.id,name:product.name,price:product.price,image:product.image,quantity:1});save()}
